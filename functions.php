...
function my_create_post_meta_box() {
	add_meta_box( 'my-meta-box', 'Ticket Link', 'my_post_meta_box', 'post', 'normal', 'high' );
}

function my_post_meta_box( $object, $box ) { ?>
	<p>
		<label for="ticket-link">Buy Ticket Link</label>
		<br />
		<input name="ticket-link" id="ticket-link" cols="60" rows="4" tabindex="30" style="width: 97%;"><?php echo wp_specialchars( get_post_meta( $object->ID, 'Ticket Link', true ), 1 ); ?></input>
...
function my_save_post_meta_box( $post_id, $post ) {
	...
	$meta_value = get_post_meta( $post_id, 'Ticket Link', true );
	$new_meta_value = stripslashes( $_POST['ticket-link'] );

	if ( $new_meta_value && '' == $meta_value )
		add_post_meta( $post_id, 'Ticket Link', $new_meta_value, true );

	elseif ( $new_meta_value != $meta_value )
		update_post_meta( $post_id, 'Ticket Link', $new_meta_value );

	elseif ( '' == $new_meta_value && $meta_value )
		delete_post_meta( $post_id, 'Ticket Link', $meta_value );
}
/** END OF CUSTOM CODE **/
